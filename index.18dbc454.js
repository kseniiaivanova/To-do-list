...
let todoThree = new Todo(Date.now(), "Pack my things", false); */ let todos = [];
...
addButton.addEventListener("click", function() {
    ...
    let userInput = new (0, _todo.Todo)(Date.now(), inputText, false);
    //kolla om inputruta inte är tom
    if (inputText.length == "0") alert("Please, enter a task!");
    else {
...
container.appendChild(header); */ function displayTodos() {
    ...
    for(let i = 0; i < todos.length; i++){
        ...
        check.addEventListener("click", ()=>{
            if (todos[i].done === false) todos[i].done = true;
...
function getListFromStorage() {
    ...
    for(let i = 0; i < storageList.length; i++){
        let newItem = new (0, _todo.Todo)(storageList[i].id, storageList[i].content, storageList[i].done);
        todos.push(newItem);
    }
    displayTodos();
}
...
},{}]},["jQVXF","1SICI"], "1SICI", "parcelRequire94c2")
